...
export default function AdminStays() {
  ...
  React.useEffect(() => {
    ...
    let mounted = true
      ; (async () => {
        try {
          const res = await fetch('/api/residence', { headers: { Authorization: `Bearer ${token}` } })
          if (res.status === 401) {
  ...
  function edit(item) {
    setForm({
      ...
      rate: item.rate || '',
      specs: {
        bedrooms: item.specs?.bedrooms || '',
        bathrooms: item.specs?.bathrooms || '',
        guests: item.specs?.guests || '',
        sqft: item.specs?.sqft || '',
        type: item.specs?.type || 'Apartment'
      },
  ...
  async function handleSubmit(e) {
    ...
    const url = editingId ? `/api/residence/${editingId}` : '/api/residence'
    try {
  ...
  async function remove(id) {
    if (!confirm('Delete this residence record?')) return
    setError('')
    const token = localStorage.getItem('admin_token')
    try {
      const res = await fetch(`/api/residence/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <AdminLayout title="Elite Residences Manager">
      <div style={{ padding: '0 40px 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 48, borderBottom: '1px solid #f1f5f9', paddingBottom: 24 }}>
          <div>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: '#0f172a', margin: 0 }}>Residential Assets</h2>
            <p style={{ color: '#64748b', fontSize: 13, marginTop: 4 }}>Manage luxury villas, suites and penthouses</p>
          </div>
      ...
      {loading && <div style={{ padding: 120, textAlign: 'center', color: '#64748B', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 2 }}>Synchronizing Collection...</div>}
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: 80, border: '1px dashed #E2E8F0', borderRadius: 24 }}>
          <Home size={48} style={{ color: '#E2E8F0', marginBottom: 16 }} />
          <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0F172A', margin: 0 }}>Empty Collection</h3>
        </div>
      )}
          ...
          {filtered.map(item => (
            <div key={item.id} className="admin-list-row" style={{ 
               ...
               <div>
                  ...
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginTop: 2 }}>{String(item.id).toUpperCase()}</div>
               </div>
               <div style={{ fontSize: 13, fontWeight: 600, color: '#475569' }}>
                  {item.category || 'Luxury Residence'}
               </div>
               <div>
      ...
      <Modal open={showCreate} title={editingId ? "Update Residence" : "Register Residence"} onClose={() => setShowCreate(false)}>
        <form onSubmit={handleSubmit} style={{ padding: '20px 0' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
            <div className="field-group">
              <label style={{ fontSize: 10, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1.5 }}>Residence Name</label>
            ...
            <div className="field-group">
              ...
              <select name="category" value={form.category} onChange={onChange} style={{ width: '100%', height: 44, borderRadius: 8, border: '1px solid #E2E8F0', padding: '0 12px', outline: 'none', background: '#fff', fontSize: 14, fontWeight: 500 }}>
                ...
                <option value="Premium Residences">Premium Residences</option>
          ...
          <div className="field-group" style={{ marginTop: 32 }}>
            <label style={{ fontSize: 10, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1.5 }}>Description</label>
          ...
          <div className="field-group" style={{ marginTop: 32 }}>
            <label style={{ fontSize: 10, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1.5 }}>Amenities (Comma separated)</label>
            <input name="features" value={form.features} onChange={onChange} placeholder="e.g. WiFi, Pool, Gym, Smart TV" style={{ width: '100%', height: 44, borderRadius: 8, border: '1px solid #E2E8F0', padding: '0 12px', outline: 'none', fontSize: 14, fontWeight: 500 }} />
          </div>

          <div style={{ marginTop: 32, padding: 24, background: '#F8FAFC', borderRadius: 16 }}>
             <h4 style={{ margin: '0 0 20px', fontSize: 12, fontWeight: 800, color: '#24276F', textTransform: 'uppercase', letterSpacing: 1.5 }}>Property Specifications</h4>
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
                <div className="field-group">
                  <label style={{ fontSize: 9, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 6 }}>BEDROOMS</label>
                  <input type="number" name="bedrooms" value={form.specs?.bedrooms} onChange={onSpecChange} style={{ width: '100%', height: 38, borderRadius: 6, border: '1px solid #E2E8F0', padding: '0 10px', fontSize: 13 }} />
                </div>
                <div className="field-group">
                  <label style={{ fontSize: 9, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 6 }}>BATHROOMS</label>
                  <input type="number" name="bathrooms" value={form.specs?.bathrooms} onChange={onSpecChange} style={{ width: '100%', height: 38, borderRadius: 6, border: '1px solid #E2E8F0', padding: '0 10px', fontSize: 13 }} />
                </div>
                <div className="field-group">
                  <label style={{ fontSize: 9, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 6 }}>GUESTS</label>
                  <input type="number" name="guests" value={form.specs?.guests} onChange={onSpecChange} style={{ width: '100%', height: 38, borderRadius: 6, border: '1px solid #E2E8F0', padding: '0 10px', fontSize: 13 }} />
                </div>
                <div className="field-group">
                  <label style={{ fontSize: 9, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 6 }}>PROPERTY TYPE</label>
                  <select name="type" value={form.specs?.type} onChange={onSpecChange} style={{ width: '100%', height: 38, borderRadius: 6, border: '1px solid #E2E8F0', padding: '0 10px', fontSize: 13, background: '#fff' }}>
                ...
                <div className="field-group">
                  <label style={{ fontSize: 9, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 6 }}>SQFT</label>
                  <input name="sqft" value={form.sqft} onChange={onChange} placeholder="e.g. 1200" style={{ width: '100%', height: 38, borderRadius: 6, border: '1px solid #E2E8F0', padding: '0 10px', fontSize: 13 }} />
                </div>
             </div>
          </div>

          <div className="field-group" style={{ marginTop: 32 }}>
            <label style={{ fontSize: 10, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1.5 }}>Hero Image</label>
          ...
          <div className="field-group" style={{ marginTop: 32 }}>
            <label style={{ fontSize: 10, fontWeight: 800, color: '#64748B', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1.5 }}>Gallery</label>
            <label style={{ width: '100%', height: 100, border: '2px dashed #E2E8F0', borderRadius: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', gap: 8 }}>
               ...
               <span style={{ fontSize: 12, fontWeight: 700, color: '#64748B' }}>ADD IMAGES</span>
          ...
          <div style={{ marginTop: 48 }}>
            <button type="submit" style={{ width: '100%', height: 48, borderRadius: 12, background: '#24276F', color: '#fff', border: 'none', fontWeight: 700, fontSize: 13 }}>{editingId ? 'Update Record' : 'Register Residence'}</button>
          </div>
        </form>
      </Modal>

      <Modal open={!!viewItem} title="Residence Dossier" onClose={() => setViewItem(null)}>
        {viewItem && (
          <div style={{ padding: '12px 0' }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 32, color: '#24276F', margin: '0 0 16px' }}>{viewItem.name}</h2>
            
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, marginBottom: 32, padding: 20, background: '#F8FAFC', borderRadius: 16 }}>
               <div>
                  <div style={{ fontSize: 9, fontWeight: 800, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: 1 }}>Bedrooms</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#334155' }}>{viewItem.specs?.bedrooms || '0'}</div>
               </div>
               <div>
                  <div style={{ fontSize: 9, fontWeight: 800, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: 1 }}>Bathrooms</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#334155' }}>{viewItem.specs?.bathrooms || '0'}</div>
               </div>
               <div>
                  <div style={{ fontSize: 9, fontWeight: 800, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: 1 }}>Guests</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#334155' }}>{viewItem.specs?.guests || '0'}</div>
               </div>
            </div>

            <div style={{ marginBottom: 32 }}>
               <div style={{ fontSize: 9, fontWeight: 800, color: '#24276F', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>Description</div>
               <p style={{ color: '#475569', lineHeight: 1.8, fontSize: 14, margin: 0 }}>{viewItem.desc || 'No summary available.'}</p>
            </div>

            {viewItem.features && viewItem.features.length > 0 && (
               <div style={{ marginBottom: 32 }}>
                  <div style={{ fontSize: 9, fontWeight: 800, color: '#24276F', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>Amenities</div>
