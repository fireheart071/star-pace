...
/**
 * Parsed helper to delete an image by its client URL if hosted on S3.
 */
...
/**
 * Parsed helper to delete multiple images from S3.
 */
export async function deleteS3Images(urls) {
  if (Array.isArray(urls)) {
    for (const url of urls) {
      await deleteS3Image(url);
    }
  } else if (urls) {
    await deleteS3Image(urls);
  }
}
