...
function normalizeModel(input) {
  return {
    ...
    chauffeurEnabled: typeof input.chauffeurEnabled !== 'undefined' ? (input.chauffeurEnabled === true || input.chauffeurEnabled === 'true') : true,
    gallery: Array.isArray(input.gallery) ? input.gallery.filter(Boolean) : [],
    features: Array.isArray(input.features)
      ? input.features.filter(Boolean)
      : (typeof input.features === 'string' ? input.features.split(',').map(s => s.trim()).filter(Boolean) : []),
...
function normalizeStay(input) {
  return normalizeResidence(input);
}

module.exports = { slugify, normalizeModel, normalizeResidence, normalizeStay, buildInvoiceBuffer, processImages }
