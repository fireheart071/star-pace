...
async function sendAdminOrderEmail(order) {
  ...
  if (resendApiKey) {
    try {
      ...
      const verifiedFrom = fromEmail.includes('example.com') || fromEmail.includes('atlas.com')
        ? 'onboarding@resend.dev'
        : fromEmail

      const text = `New booking request ${order.id}\nCustomer: ${order.name}\nPhone: ${order.phone || '-'}\nEmail: ${order.email || '-'}\nProduct: ${order.productName || order.product || '-'}\nPickup/Location: ${order.location || '-'}\nDates: ${order.start || '-'} to ${order.end || '-'}\nNotes: ${order.note || '-'}\n\nView in admin dashboard.`

      const data = await resend.emails.send({
        from: verifiedFrom,
        to: adminEmail,
        subject: `New booking request ${order.id} - ${order.name}`,
        text: text
      })

      return { ok: true, via: 'resend', data }
    } catch (e) {
      console.error('Admin Resend API catch Error', e)
    }
  }
  ...
  if (resendApiKey) {
    try {
      ...
      const data = await resend.emails.send({
        from: verifiedFrom,
        to: order.email,
        subject: `Your Star Pace Confirmation - Booking #${order.id}`,
        html: `<p>Hello ${order.name},</p><p>Thank you for choosing Star Pace. We have received your booking request for the <b>${order.productName}</b> (Booking #${order.id}).</p><p>Our team will contact you shortly.</p>`,
        attachments: [
          {
      ...
      console.error('Customer Resend API catch Error', e)
    }
  }
  ...
  if (resendApiKey) {
    try {
      ...
      const data = await resend.emails.send({
        from: verifiedFrom,
        to: order.email,
        subject: `Approved: Your Star Pace Booking - Booking #${order.id}`,
        html: `<p>Hello ${order.name},</p><p>We are pleased to inform you that your booking request (Booking #${order.id}) for the <b>${order.productName}</b> has been <b>APPROVED</b>.</p><p>Please find your official invoice attached to this email.</p><p>Scheduled Dates: ${order.start} to ${order.end}</p><br/><p>Best regards,<br/>Star Pace Team</p>`,
        attachments: [
          {
            filename: `invoice-${order.id}.pdf`,
            content: invoiceBuffer
      ...
      console.error('Approval Resend API catch Error', e)
    }
  }
...
async function scheduleCustomerFeedbackSms(order) {
  if (!order.phone || !order.end) return { ok: false, skipped: true, reason: 'Missing phone or return date' }
  const provider = (process.env.SMS_PROVIDER || 'paystack').toLowerCase()
  if (provider !== 'arkesel') {
    return { ok: false, skipped: true, reason: 'Feedback scheduling only supported on Arkesel' }
  }
  ...
  const dateParts = order.end.split('-')
  if (dateParts.length !== 3) return { ok: false, skipped: true, reason: 'Invalid return date format' }
  const scheduleDate = `${dateParts[2]}-${dateParts[1]}-${dateParts[0]} 12:00 PM`

  const message = `Hello ${order.name}, thank you for choosing Star Pace. We hope you had a wonderful experience! Please take a moment to leave a review: https://starpace.up.railway.app/review?bookingId=${order.id}`
  ...
  try {
    ...
    console.error('Failed to schedule feedback SMS', e)
    return { ok: false, error: e.message }
  }
}
