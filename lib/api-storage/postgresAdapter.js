...
if (!DATABASE_URL || typeof DATABASE_URL !== 'string' || !DATABASE_URL.trim()) {
  throw new Error('Postgres adapter requires DATABASE_URL environment variable')
}
...
async function ensureTables() {
  ...
  try {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS residences (id TEXT PRIMARY KEY, data JSONB)`)
    await client.query(`CREATE TABLE IF NOT EXISTS reviews (id TEXT PRIMARY KEY, data JSONB)`)
...
module.exports = {
  getReviews: async () => await getRows('reviews'),
  saveReviews: async (rows) => await saveRows('reviews', rows),
  deleteReview: async (id) => await deleteRow('reviews', id),
  ...
  getResidences: async () => await getRows('residences'),
  saveResidences: async (rows) => await saveRows('residences', rows),
  deleteResidence: async (id) => await deleteRow('residences', id)
}
